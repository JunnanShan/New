...
#pragma HLS PIPELINE
							inp_image_local[Ci][Ri/(INP_IMAGE_SIZE-2*PAD)+PAD][Ri%(INP_IMAGE_SIZE-2*PAD)+PAD] = inp_img[(ti+Ci+batch*INP_IMAGE_CHANNEL/GROUP)*IMAGE + Ri];
						}
					}
...
#pragma HLS PIPELINE
							filter_local[Bi*Tn+Ci/9][Ci%9] = filter[(Bi+to+batch*FILTER_BATCH/GROUP)*FILTER +ti*FILTER_SIZE*FILTER_SIZE + Ci];
						}
					}
