...
void conv2(DataType inp_img[(INP_IMAGE_SIZE-2*PAD) * (INP_IMAGE_SIZE-2*PAD) * INP_IMAGE_CHANNEL],
...
int main()
{
    ifstream inp_file("/home/junnan/Work/Vivado_HLS/conv2_final/out_norm1.txt");
    DataType *inp_image;
	...
	if(inp_file.is_open())
	{
		...
		for (int i=0; i<(INP_IMAGE_SIZE-2*PAD) * (INP_IMAGE_SIZE-2*PAD) * INP_IMAGE_CHANNEL; i++)
		{
	...
	ifstream filter_file("/home/junnan/Work/Vivado_HLS/conv2_final/conv2Weights.txt");
	    DataType *filter;
		...
		if(filter_file.is_open())
		{
			...
			for (int i=0; i<FILTER_BATCH*FILTER_CHANNEL*FILTER_SIZE*FILTER_SIZE; i++)
			{
		...
		ifstream bias_file("/home/junnan/Work/Vivado_HLS/conv2_final/conv2Bias.txt");
		    DataType *bias;
  ...
  ofstream data("/home/junnan/Work/Vivado_HLS/conv2_final/out_image.txt");
