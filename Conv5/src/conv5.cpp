...
void conv5(DataType inp_img[(INP_IMAGE_SIZE-2*PAD) * (INP_IMAGE_SIZE-2*PAD) * INP_IMAGE_CHANNEL],
...
L3:		for(int batch=0; batch<2; batch++){
L4:			for(int to=0; to<FILTER_BATCH/GROUP; to+=Tm){//chan of output image
	DataType out[Tm][Tr][Tc];
	#pragma HLS ARRAY_PARTITION variable=out complete dim=1
L5:				for(int ti=0; ti<INP_IMAGE_CHANNEL/GROUP; ti+=Tn){//chan of input image

	DataType inp_image_local[Tn][Tr+2][Tc+2];
...
I2:						for(int Ri=0; Ri<INP_IMAGE_SIZE-2; Ri++){
I3:							for(int Li=0; Li<INP_IMAGE_SIZE-2; Li++){
#pragma HLS PIPELINE
								inp_image_local[Ci][Ri+1][Li+1] = inp_img[(Ci+ti+batch*INP_IMAGE_CHANNEL/GROUP)*IMAGE + Ri*(INP_IMAGE_SIZE-2) + Li];
							}
						}
					}
...
W3:							for(int Ri=0; Ri<FILTER_SIZE; Ri++){
W4:								for(int Li=0; Li<FILTER_SIZE; Li++){
#pragma HLS PIPELINE
									filter_local[Bi*Tn+Ci][Ri][Li] = filter[(Bi+to+batch*FILTER_BATCH/GROUP)*FILTER + (Ci+ti)*FILTER_SIZE*FILTER_SIZE + Ri*FILTER_SIZE + Li];
								}
							}
						}
					}
...
N6:										for(int tii=0; tii<Tn; tii++){//chan tile of input image
											out[too][trr][tcc] += filter_local[too*Tn+tii][i][j] * inp_image_local[tii][STRIDE*trr+i][STRIDE*tcc+j];
										}
									}
								}
							}
